...
var data=`Hostname,Role,in,SRL,Network,Host,IP,Address,OS,Mem,Size,OS,Version/Volatility,Profile
base-rd-01,TDungan's,workstation,172.16.6.11,3.0,GB,Win10x64_16299
base-rd-02,R&D,user's,workstation,172.16.6.12,3.0,GB,Win10x64_16299
base-rd-03,R&D,user's,workstation,172.16.6.13,3.0,GB,Win10x64_16299
base-rd-04,NRomanoff's,workstation,172.16.6.14,3.0,GB,Win10x64_16299
base-rd-05,R&D,user's,workstation,172.16.6.15,3.0,GB,Win7SP1x64
base-rd-06,R&D,user's,workstation,172.16.6.16,3.0,GB,Win7SP1x64
base-wkstn-01,MHill's,workstation,172.16.7.11,3.0,GB,Win10x64_16299
base-wkstn-02,Business,user's,workstation,172.16.7.12,3.0,GB,Win10x64_16299
base-wkstn-03,Business,user's,workstation,172.16.7.13,3.0,GB,Win10x64_16299
base-wkstn-04,Business,user's,workstation,172.16.7.14,3.0,GB,Win10x64_16299
base-wkstn-05,NFury's,workstation,172.16.7.15,3.0,GB,Win7SP1x64
base-wkstn-06,Business,user's,workstation,172.16.7.16,3.0,GB,Win7SP1x64
base-admin,RSydow's,workstation,172.16.5.26,4.0,GB,Win10x64_15063
base-av,McAfee,AV,ePo,server,172.16.5.20,8.0,GB,Win2012R2x64
base-dc,Domain,Controller,(2016,DFL),172.16.4.4,4.0,GB,Win2016x64_14393
base-elf,Event,Log,Forwarding,server,172.16.5.21,4.0,GB,Win2016x64_14393
base-file,File,server,172.16.4.5,2.0,GB,Win2012R2x64
base-hunt,CBarton's,workstation,172.16.5.25,4.0,GB,Win10x64_14393
base-mail,Exchange,server,172.16.4.6,16.0,GB,Win2016x64_14393
base-sp,SharePoint,server,172.16.4.7,8.0,GB,Win2012R2x64
dmz-ftp,DMZ,FTP,server,172.16.10.12,4.0,GB,Win2012R2x64
dmz-www,DMZ,web,server,172.16.10.13,2.0`
...
function isIP(e){
  expression = /(?:(?:\d|[01]?\d\d|2[0-4]\d|25[0-5])\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d|\d)(?:\/\d{1,2})?/gi;
  var regexp = new RegExp(expression);
  return e.match(regexp);
}
function lookup_ip(word){
  var str = getVal(word);
  console.log(str)
  console.log("TEST")
  var data_list = data.split('\n')
  for (let i = 0; i < data_list.length; i++) {
    var item = data_list[i].split(',')
    for (let j = 0; j < item.length; j++) {
      if(item[j]==str){
        window.alert(data_list[0]+"\n"+item)
      }
    }
    
  }
}
function lookup_port(word){
  var str = getVal(word);
  console.log(str)
  var data_list = ports.split('\n')
  for (let i = 0; i < data_list.length; i++) {
    var item = data_list[i].split(',')
    for (let j = 0; j < item.length; j++) {
      if(item[j]==str){
...
var context = ["selection","link","editable"];
chrome.contextMenus.create({"title": "Lookup IP", "contexts":context,"onclick": lookup_ip});
chrome.contextMenus.create({"title": "Lookup Port", "contexts":context,"onclick": lookup_port});
